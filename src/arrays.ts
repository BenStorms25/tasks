...
export const removeDollars = (amounts: string[]): number[] => {
    ...
    //         newArray[i] = 0;
    //     } else {
    //         newArray[i] = parseInt(amounts[i]);
    //     }
    // }
    // console.log(newArray);
    newArray = amounts.map((amount) => {
        if (amount.includes("$")) {
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    ...
    const arr = upperCase.filter((message) => !message?.includes("?"));
...
export function countShortWords(words: string[]): number {
    const lessThanFour = words.filter((word) => word.length < 4);
...
export function makeMath(addends: number[]): string {
    if (addends.length == 0) return "0=0";
        ...
        addends.forEach((number) => {
            thisString += String(number) + "+";
        });
        let sum = addends.reduce((accumulator, value) => {
            return accumulator + value;
        }, 0);
        thisString = thisString.slice(0, -1);
        thisString = sum + thisString;
        return thisString;
    }
}
