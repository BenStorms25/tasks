...
export function toShortForm(question: Question): string {
    let thisString = question.id + ": " + question.name.slice(0, 10);
    return thisString;
}
...
export function toMarkdown(question: Question): string {
    let currentString = "# " + question.name + "\n" + question.body;
...
export function publishQuestion(question: Question): Question {
    if (question.name.includes("Addition")) {
...
export function mergeQuestion(
    ...
    let mergedQuestion = contentQuestion;
    mergedQuestion.points = points;
    mergedQuestion.name = name;
    mergedQuestion.id = id;
    if (mergedQuestion.options.includes("heptagon")) {
        mergedQuestion.options.pop();
    }
    if (mergeQuestion.name === "Less Points Shape") {
        mergedQuestion.points = 1;
    }
